 ...
 */
public fun Route.upgradeScriptCollectionDesigntimeArtifactsByIdAndVersion(action: suspend ApplicationCall.(ScriptCollectionDesigntimeArtifactUpdate) -> Unit) {
  route(path = """/ScriptCollectionDesigntimeArtifacts(Id='{id}',Version='{version}')""") {
    contentType(Json) {
      put {
        val body = call.receive<ScriptCollectionDesigntimeArtifactUpdate>()
        call.action(body)
        call.respond(OK)
      }
    }
