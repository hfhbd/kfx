 ...
 */
public fun Route.getIntegrationDesigntimeArtifactsByIdAndVersionValue(action: suspend ApplicationCall.() -> Source) {
  route(path = """/IntegrationDesigntimeArtifacts(Id='{id}',Version='{version}')/{$}value""") {
    contentType(Json) {
      accept(Zip) {
        `get` {
