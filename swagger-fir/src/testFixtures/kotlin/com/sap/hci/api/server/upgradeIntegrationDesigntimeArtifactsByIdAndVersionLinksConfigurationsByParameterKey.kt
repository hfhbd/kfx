 ...
 */
public fun Route.upgradeIntegrationDesigntimeArtifactsByIdAndVersionLinksConfigurationsByParameterKey(action: suspend ApplicationCall.(ConfigurationUpdate) -> Unit) {
  route(path = """/IntegrationDesigntimeArtifacts(Id='{id}',Version='{version}')/{$}links/Configurations('{parameterKey}')""") {
    contentType(Json) {
      put {
        val body = call.receive<ConfigurationUpdate>()
        call.action(body)
