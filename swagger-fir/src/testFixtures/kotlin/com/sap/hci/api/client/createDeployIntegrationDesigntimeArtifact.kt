 ...
 * @param id Id of integration artifact - enclosed in single quotes <br>
 * Example: ```'IntegrationFlowWithConfiguration'```
 * @param version Version of integration artifact - enclosed in single quotes  <br>
 * You can enter either ```'active'``` or the version name (e.g. ```'1.0.5'```) for retrieving the current version.
 */
@Throws(Error::class)
public suspend fun HttpClient.createDeployIntegrationDesigntimeArtifact(
  X_CSRF_Token: String,
  id: String? = null,
  version: String? = "'active'",
  builder: suspend HttpRequestBuilder.() -> Unit = {},
) {
  val response = post(urlString = """DeployIntegrationDesigntimeArtifact""") {
