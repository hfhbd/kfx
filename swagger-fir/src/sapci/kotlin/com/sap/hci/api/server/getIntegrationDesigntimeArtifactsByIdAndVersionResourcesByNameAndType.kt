 ...
 */
public fun Route.getIntegrationDesigntimeArtifactsByIdAndVersionResourcesByNameAndType(action: suspend ApplicationCall.() -> GetIntegrationDesigntimeArtifactsByIdAndVersionResourcesByNameAndType) {
  route(path = """/IntegrationDesigntimeArtifacts(Id='{id}',Version='{version}')/Resources(Name='{name}',ResourceType='{type}')""") {
    contentType(Json) {
      accept(Json) {
        `get` {
