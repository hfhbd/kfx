 ...
 */
public fun Route.getMessageMappingDesigntimeArtifactsByIdAndVersionValue(action: suspend ApplicationCall.() -> Source) {
  route(path = """/MessageMappingDesigntimeArtifacts(Id='{id}',Version='{version}')/{$}value""") {
    contentType(Json) {
      accept(Zip) {
        `get` {
